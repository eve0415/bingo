...
- **The host, the player and the watcher get different screens.** A player sees the call and their own card and nothing that competes with them. The host reads a flashboard, a roster with reach and bingo counts, any player's card on demand, and their own card if they took one; drawing is their one primary action. Somebody in the room without a card gets the caller's screen without the controls: what there is to watch is other people's cards, so they are what a watcher is given, the same way the host is given them.
- **A seat is not the room.** Holding a card and being in the game are separate, and the control that changes one says what it would do rather than what is true now — 参加する, 参加しない. On a card screen it sits beside the roster and never next to the board, because nothing a thumb can reach mid-tap should be able to take a card away.
